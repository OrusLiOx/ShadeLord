...
public class Attacks : MonoBehaviour
{
	...
	private bool wait, forward, infiniteSpike;
	...
	public void SweepBeam()
	{
		...
		{
			// go to side
			bool goright = UnityEngine.Random.Range(0, 1f) > .5f, mid = false;
	...
	public void AimBeam()
	{
		...
		IEnumerator AimBeam(int beams)
		{
			...
			for (int i =0;i<beams;i++)
			{
				...
				if (beams == 4 && Math.Abs(x2-transform.position.x) < 7f && y2-15f<transform.position.y)
				{
					stop = true;
					atts["BeamOrigin"].SetActive(false);
					eyes(true);
					...
					leave();
					yield return new WaitUntil(() => !wait);
					StartCoroutine(AimBeam(2));
					break;
				}

				// targeting
				float x = (beam.transform.position.x + 0.91f) - (x2 + x2 - x1);
				float y = (beam.transform.position.y + .7f) - (y2 + y2 - y1);
